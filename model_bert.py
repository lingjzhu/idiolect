...
if __name__ == "__main__":
    ...
    if args.load_model:
        ...
        if args.attpool == True:
            model = SBert(args.model).to(device)
        else:
            model = SBert_vanilla(args.model).to(device)    
    ...
    if args.loss == 'margin':
        model_name = "bert-%s-%s-%s-mask-%s"%(args.distance,args.tau_low,args.tau_high,args.mask_prob)
    elif args.loss == 'anchor':
        model_name = "bert-%s-%s-mask-%s-delta-%s-alpha-%s"%(args.distance,args.loss,args.mask_prob,args.delta,args.alpha)
    elif args.loss == 'modified_anchor':
        model_name = "bert-%s-%s-mask-%s-delta-%s-%s-alpha-%s"%(args.distance,args.loss,args.mask_prob,args.tau_low,args.tau_high,args.alpha)
    elif args.loss == 'margin_anchor':
        model_name = "bert-%s-%s-mask-%s-delta-%s-%s-alpha-%s"%(args.distance,args.loss,args.mask_prob,args.tau_low,args.tau_high,args.alpha)
